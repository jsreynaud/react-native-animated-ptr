...
class AnimatedPTR extends React.Component {
  PTR_ScrollComponent = React.createRef();
  ...
  onScrollRelease() {
    if (!this.props.isRefreshing && this.state.shouldTriggerRefresh) {
      if (this.PTR_ScrollComponent.current) {
        this.PTR_ScrollComponent.current.scrollTo({ y: -this.props.minPullDistance })
      }
  ...
  UNSAFE_componentWillReceiveProps(props) {
    if (this.props.isRefreshing !== props.isRefreshing) {
      if (!props.isRefreshing) {
        if (this.PTR_ScrollComponent.current) {
          this.PTR_ScrollComponent.current.scrollTo({ y: 0 });
        }
