...
class AnimatedPTR extends React.Component {
  ...
  _handleMoveShouldSetPanResponder(e, gestureState) {
    return !this.state.isScrollFree && (Math.abs(gestureState.dx) + Math.abs(gestureState.dx)) > 1.5;
  }
  ...
  _handlePanResponderMove(e, gestureState) {
    if (!this.props.isRefreshing) {
      if ((gestureState.dy >= 0 && this.state.scrollY._value === 0) || this.state.refreshHeight._value > 0) {
        ...
        if (this.PTR_ScrollComponent.current) {
          this.PTR_ScrollComponent.current.scrollTo({ y: -1 * gestureState.dy, animated: true });
        }
      }
    }
  }
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: this.props.contentBackgroundColor }}
        ...
        <ScrollView ref={this.PTR_ScrollComponent}
